...
public class SJF implements Algorithm {
	...
	public void schedule() {
		// newly made
		Collections.sort(tasks, new ScheduleSort());
...
class ScheduleSort implements Comparator<OneSchedule> {

	public int compare(OneSchedule task1, OneSchedule task2) {
		// Compare the arrival times of the tasks
		int arrivalTimeComparison = Integer.compare(task1.getArrivalTime(), task2.getArrivalTime());

		// If the arrival times are the same, sort by CPU burst time
		if (arrivalTimeComparison == 0) {
			return Integer.compare(task1.getCpuBurst(), task2.getCpuBurst());
		}
		// If the arrival times are different, sort by arrival time
		else {
